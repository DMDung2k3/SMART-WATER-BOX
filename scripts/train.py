...
# [Nội dung mô hình: conv_bn_leaky, DigitCNN, YOLOv3TinyWithCNN, WaterMeterDataset, custom_collate_fn, YOLOLoss, CombinedLoss]
# Sao chép từ code bạn cung cấp
...
def train(args):
    ...
    try:
        train_img_dir = check_directory(os.path.join(args.data_path, 'train', 'images'), "Train images directory")
        train_label_dir = check_directory(os.path.join(args.data_path, 'train', 'labels'), "Train labels directory")
        valid_img_dir = check_directory(os.path.join(args.data_path, 'valid', 'images'), "Valid images directory")
        valid_label_dir = check_directory(os.path.join(args.data_path, 'valid', 'labels'), "Valid labels directory")
    except FileNotFoundError as e:
        print(f"Error: {e}")
        return

    # Define anchors (example values, adjust based on dataset)
    anchors = [
    ...
    # Initialize model
    model = YOLOv3TinyWithCNN(num_classes_yolo=1, num_classes_cnn=10).to(device)
    optimizer = optim.Adam(model.parameters(), lr=args.lr)
    criterion = CombinedLoss(num_classes_yolo=1, num_classes_cnn=10, anchors=anchors, img_size=args.img_size)
    ...
    try:
        ...
        valid_dataset = WaterMeterDataset(
    ...
    for epoch in range(args.epochs):
        ...
        for images, yolo_boxes, digit_labels in train_loader:
            images = images.to(device)
            yolo_boxes = [boxes.to(device) for boxes in yolo_boxes]
            digit_labels = [labels.to(device) for labels in digit_labels]

            optimizer.zero_grad()
            det_small, det_medium, det_large, cnn = model(images)
            loss = criterion(det_small, det_medium, det_large, cnn, images, yolo_boxes, digit_labels, anchors)
            loss.backward()
            optimizer.step()
        ...
        with torch.no_grad():
            for images, yolo_boxes, digit_labels in valid_loader:
                images = images.to(device)
                yolo_boxes = [boxes.to(device) for boxes in yolo_boxes]
                digit_labels = [labels.to(device) for labels in digit_labels]
                det_small, det_medium, det_large, cnn = model(images)
        ...
        print(f"Epoch {epoch+1}/{args.epochs}, Train Loss: {epoch_train_loss:.4f}, Valid Loss: {epoch_valid_loss:.4f}")

        # Save best model
        if epoch_valid_loss < best_valid_loss:
            best_valid_loss = epoch_valid_loss
            os.makedirs(os.path.join(args.output_path, 'models', 'weights'), exist_ok=True)
            torch.save(model.state_dict(), os.path.join(args.output_path, 'models', 'weights', 'yolov3tiny_cnn.pth'))
    ...
    plt.ylabel('Loss')
    plt.legend()
    plt.savefig(os.path.join(args.output_path, 'loss_plot.png'))
    plt.close()
...
if __name__ == '__main__':
    ...
    parser.add_argument('--img_size', type=int, default=420, help='Image size')

...
#from models.yolov3tiny_cnn import YOLOv3TinyWithCNN, WaterMeterDataset
import cv2
import numpy as np
...
def test(args):
    ...
    # Dataset
    dataset = WaterMeterDataset(
        img_dir=os.path.join(args.data_path, 'valid', 'images'),
        label_dir=os.path.join(args.data_path, 'valid', 'labels'),
        transform=transform,
    ...
    with torch.no_grad():
        for i, (images, _, _) in enumerate(dataloader):
            ...
            detections, digit_preds = model.extract_and_classify(images, det_small, det_medium, det_large)

            # Load original image for visualization
            img_path = dataset.img_files[i]
            img = cv2.imread(os.path.join(args.data_path, 'valid', 'images', img_path))
            img = cv2.cvtColor(img, cv2.COLOR_BGR2RGB)

            # Draw detections
            for det in detections:
                _, cls_id, x_min, y_min, x_max, y_max, score = det
                if int(cls_id) == 0:
                    x_min, y_min, x_max, y_max = map(int, [x_min, y_min, x_max, y_max])
                    cv2.rectangle(img, (x_min, y_min), (x_max, y_max), (0, 255, 0), 2)
                    cv2.putText(img, f"Score: {score:.2f}", (x_min, y_min-10), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (0, 255, 0), 2)

            # Draw digit predictions
            for digit_pred in digit_preds:
                _, digit, x_min, y_min, x_max, y_max = digit_pred
                x_min, y_min, x_max, y_max = map(int, [x_min, y_min, x_max, y_max])
                cv2.putText(img, f"Digit: {digit}", (x_min, y_max+20), cv2.FONT_HERSHEY_SIMPLEX, 0.5, (255, 0, 0), 2)
...
if __name__ == '__main__':
    ...
    parser.add_argument('--data_path', type=str, default=r'C:\Users\dung\PycharmProjects\PythonProject1\Water_meter\data', help='Path to dataset')
    parser.add_argument('--model_path', type=str, default='./yolov3tiny_cnn.pth', help='Path to model weights')
    parser.add_argument('--output_path', type=str, default='outputs/val_output', help='Output directory')
